...
func BenchmarkInitializeGameFromFEN(b *testing.B) {
	b.ReportAllocs()
	fens := []string{
...
func BenchmarkLoadPGN(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		game := &Game{}
		if err := game.LoadPGN(shortPGN); err != nil {
...
func BenchmarkLoadPGN_LongGame(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		game := &Game{}
		if err := game.LoadPGN(longPGN); err != nil {
			b.Fatalf("load pgn: %v", err)
		}
	}
}
